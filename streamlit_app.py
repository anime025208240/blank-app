...
if uploaded_base_file:
    try:
        excel_bytes = uploaded_base_file.getvalue()
        ...
        if st.button("🔄 載入工作表並執行智慧關聯"):
            with st.spinner("正在智慧搜尋標題列並進行資料解析..."):
                ...
                for skip_rows in range(10):
                    test_df = pd.read_excel(excel_bytes, sheet_name=finance_sheet, header=skip_rows, nrows=2, engine="openpyxl")
                ...
                found_info_header = 0
                for skip_rows in range(10):
                    test_df = pd.read_excel(excel_bytes, sheet_name=auto_info_sheet, header=skip_rows, nrows=2, engine="openpyxl")
                    test_cols = [str(c).strip() for c in test_df.columns]
                    if "房客" in test_cols or "房客姓名" in test_cols:
                        found_info_header = skip_rows
                        break

                df_finance = pd.read_excel(excel_bytes, sheet_name=finance_sheet, header=found_finance_header, engine="openpyxl")
                df_info = pd.read_excel(excel_bytes, sheet_name=auto_info_sheet, header=found_info_header, engine="openpyxl")
        ...
        if "距離支付日天數" in agent_df.columns:
            ...
            if not rent_reminder_df.empty:
                def get_pay_alert_memo(days):
                    if days == 0:
                        return "⏳ 今天就是繳費日！請注意確認入帳"
                    elif days == 1:
                        return "⏳ 明天即將繳費！可傳訊息提醒"
                    else:
                        return f"🔔 倒數 {int(days)} 天繳費 (請於前7天注意提醒)"
                ...
                st.dataframe(
                    rent_reminder_df[exist_pay_cols].rename(columns={"房客租金支付日_顯示": "📅 房客租金支付日", "原因_文字": "📝 欠費狀態 (原因)"}),
